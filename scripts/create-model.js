...
module.exports = (name, timestamps = true) => {
  const camelCaseName = name.split('_').map((x) => `${x[0].toUpperCase()}${x.slice(1)}`).join('');
...
`;
  ...
  if (fs.existsSync(relPath)) throw new Error(`There is already a model in path: ${relPath}`);
